...
module.exports = {
  ...
  cooldown: 5000,
  owner: true,
  description: "Save replied media to the server.",
  ...
  async run(sock, m, args, reply, chat) {
    ...
    if (!isMedia) {
      return reply("⌬ Reply to an image, video, audio, document, or sticker.");
    }
    ...
    try {
      ...
      const ext = fileNameHint
        ? path.extname(fileNameHint) || `.${extForType(targetType)}`
        : `.${extForType(targetType)}`;

      const base = (args[0] || Date.now().toString()).replace(
        /[^a-z0-9_-]/gi,
        "_",
      );
      ...
      await reply(`╰─ Failed to save file: ${err.message}`);
    }
  },
};
