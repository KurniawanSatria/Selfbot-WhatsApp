...
const EXCLUDE_DIRS = new Set(["node_modules", "session", ".npm", "tmp", ".git", "data"]);
...
module.exports = {
  ...
  cooldown: 3000,
  owner: true,
  description: "Cari file di disk bot by nama (fuzzy), lalu kirim isi sebagai text + tombol copy. .getfile <query> atau .getfile <path>",

  async run(sock, m, args, reply, chat) {
    if (!args.length) return reply("❌ Usage: `.getfile <filename>` (e.g. `message-upsert`)");
    ...
    if (rawInput.includes("/") || rawInput.includes("\\")) {
      ...
      if (!direct) {
        return reply("❌ Path tidak valid atau di luar folder bot.");
      }
    ...
    if (scored.length === 0) {
      return reply(`❌ No similar file found for "${rawInput}".`);
    }
    ...
    const btn = new Button(sock)
      .setBody(`🔎 *${scored.length} similar files* "${rawInput}":\nPilih untuk ditampilkan:`)
      .setFooter("© Saturia")
      .addSelection("Pilih File");
    for (const s of scored) {
      const relPath = "./" + path.relative(BOT_ROOT, s.f).replace(/\\/g, "/");
      btn.makeSection(path.basename(path.dirname(s.f)) || "root");
      btn.makeRow("", s.base, `${(s.score * 100).toFixed(0)}% match`, `.getfile ${relPath}`);
    }
...
async function sendFile(sock, m, chat, filePath) {
  try {
    ...
    if (stat.size > 100 * 1024 * 1024) {
      return sock.sendMessage(chat, { text: `⚠️ File too large (${sizeKB} KB) untuk dikirim.` }, { quoted: m });
    }
    ...
    if (content.length > MAX_COPY_CHARS) {
      ...
      return sock.sendMessage(
        chat,
        { document: buf, fileName: name, mimetype: "application/octet-stream", caption: `📄 ${name} (${sizeKB} KB)\n⚠️ Terlalu panjang untuk copy-text, dikirim sebagai dokumen.` },
        { quoted: m },
      );
    }
    ...
    const btn = new Button(sock)
      ...
      .setFooter("© Saturia")
      .addCopy("📋 Copy Isi File", content);

    await btn.send(chat, { quoted: m });
  } catch (err) {
    await sock.sendMessage(chat, { text: "❌ Gagal membaca/mengirim: " + err.message }, { quoted: m });
  }
}