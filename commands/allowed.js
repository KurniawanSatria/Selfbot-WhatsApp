    ...
    async run(sock, m, args, reply, chat) {
        ...
        if (!subCommand || subCommand === 'help') {
            ...
            await new Button(sock)
                .setBody(helpText)
                .setFooter('© Saturiaaa')
                .addReply('NUMBERS', 'allowed numbers')
                .addReply('CHATS', 'allowed chats')
                .addReply('SETTINGS', 'allowed settings')
        ...
        if (subCommand === 'chats' || subCommand === 'groups') {
            ...
            text += `🟢 = Enabled\n🔴 = Disabled\n\n`;
            text += `_Pilih chat di daftar untuk toggle status_`;

            const listTitle = subCommand === 'groups' ? 'Group Chats' : 'All Chats';
            const btn = new Button(sock)
                .setBody(text)
                .setFooter('© Saturiaaa')
                .addSelection(listTitle)
                .makeSection(listTitle);

            chats.forEach((c, i) => {
                btn.makeRow(
                    '',
                    `${c.enabled ? '🟢' : '🔴'} ${i + 1}. ${c.chatName || 'Unknown'}`,
                    `${c.chatType} • ${c.enabled ? 'Enabled' : 'Disabled'}`,
                    `allowed toggle ${c.chatId} ${c.enabled ? 'disable' : 'enable'}`
                );
            });
