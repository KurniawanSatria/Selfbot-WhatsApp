...
module.exports = {
  ...
  async run(sock, m, args, reply, chat) {
    ...
    if (!text || !text.trim()) return reply("❌ Reply to a text message, or type:\n`.topdf <text to convert to PDF>`");
    text = text.trim();
    try {
      ...
      await sock.sendMessage(chat, { document: { url: outFile }, fileName: path.basename(outFile), mimetype: "application/pdf", caption: 'Here is your PDF'},{ quoted: m },);
      fs.unlink(outFile, () => {});
    } catch (err) {
