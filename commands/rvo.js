...
module.exports = {
  ...
  async run(sock, m, args, reply) {
    if (!m.message?.extendedTextMessage?.contextInfo?.quotedMessage)
      return reply("╰─ Reply to a view-once message first.");
    ...
    if (!voTypes.includes(quotedType))
      return reply("❌ Not a view-once message (image/video/audio)!");
    ...
    try {
      const buffer = await downloadMediaMessage(
        ...
        "buffer",
        {},
        { logger: global.logger, reuploadRequest: sock.updateMediaMessage },
      );
