...
function serialize(sock, m) {
  ...
  m.sender = m.isGroup
    ? cleanJid(sock, m.key.participant, m.key.participantAlt)
    : cleanJid(sock, m.key.remoteJid, m.key.remoteJidAlt);
  ...
  if (m.mtype === "protocolMessage" && m.msg?.key) {
    ...
    m.msg.key.participant =
      cleanJid(sock, m.msg.key.participant, m.msg.key.participantAlt) ||
      "status_me";
    m.msg.key.fromMe = m.msg.key.participant === sock.user?.id;
    if (!m.msg.key.fromMe && m.msg.key.remoteJid === sock.user?.id)
  ...
  if (rawQuoted) {
    ...
    const qFakeObj = M.fromObject({
      ...
      message: rawQuoted,
      ...(m.isGroup ? { participant: qParticipant } : {}),
    });
    m.quoted = {
      ...qmsg,
      mtype: qtype,
      ...
      fromMe: qParticipant === sock.user?.id,
      isBaileys:
        m.msg.contextInfo?.stanzaId?.startsWith("BAE5") &&
        m.msg.contextInfo?.stanzaId?.length === 16,
      text: rawQText,
      body: rawQText,
      ...
      copyNForward: (jid, force = false, opts = {}) =>
        sock.copyNForward(jid, qFakeObj, force, opts),
    };
  } else m.quoted = null;
  ...
  m.reply = (text, options = {}) =>
    sock.sendMessage(
      m.chat,
      { text, secureMetaServiceLabel: true, ...options },
      { quoted: m },
    );
  m.react = (emoji) =>
