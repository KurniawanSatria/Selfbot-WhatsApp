  ...
  await sock.sendMessage(
    jid,
    {
      ...
      ...(ptt
        ? {
            waveform: [
              1, 18, 32, 49, 70, 61, 50, 81, 64, 77, 71, 85, 62, 52, 84, 68, 86,
              62, 84, 74, 50, 80, 77, 65, 100, 61, 73, 72, 69, 76, 99, 76, 87,
              67, 83, 84, 53, 96, 84, 81, 61, 78, 83, 53, 94, 87, 82, 74, 83,
              83, 49, 81, 94, 85, 74, 88, 89, 60, 82, 90, 78, 46, 45, 7,
            ],
          }
        : {}),
    },
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code === 0) resolve(result);
        ...
        reject(
          Object.assign(new Error(`${file} exited with code ${code}`), result),
        );
    });
  });
}
...
async function tmpFile(ext) {
  ...
  return path.join(
    STK_TMP,
    `${randomBytes(10).toString("hex")}${ext ? "." + ext : ""}`,
  );
}
...
async function sendStickerPack(sock, jid, data = {}, options = {}) {
  ...
  if (!stickers.length)
    throw new Error("sendStickerPack: stickers tidak boleh kosong");
  if (!cover) throw new Error("sendStickerPack: cover wajib diisi");

  const encryptForUpload = async (
    plaintext,
    mediaType,
    providedMediaKey = null,
  ) => {
    const mediaKey = providedMediaKey
      ? Buffer.from(providedMediaKey)
      : randomBytes(32);
  ...
  const writeTempAndUpload = async (encrypted, mediaType) => {
    ...
    try {
      ...
      if (!result?.directPath)
        throw new Error(`Upload ${mediaType} gagal: tidak ada directPath`);
      return result;
    } finally {
  ...
  for (let i = 0; i < stickers.length; i++) {
    const s = stickers[i];
    const srcBuf = await resolveBuffer(
      s.data ?? s.sticker ?? s.url ?? s.path ?? s,
    );
    if (!srcBuf) {
      console.warn(
        `[sendStickerPack] sticker #${i + 1} dilewati (buffer kosong)`,
      );
      continue;
    }
    const webpBuf = await toWebp(srcBuf);
    const fileName = `${createHash("sha256").update(webpBuf).digest("base64url")}.webp`;
    if (!zipEntries.some((e) => e.name === fileName))
      zipEntries.push({ name: fileName, data: webpBuf });
    stickerMeta.push({
      fileName,
      mimetype: "image/webp",
      ...
      isLottie: false,
      emojis:
        Array.isArray(s.emojis) && s.emojis.length ? s.emojis : defaultEmojis,
      accessibilityLabel: s.accessibilityLabel || s.label || "",
    });
  }

  if (!stickerMeta.length)
    throw new Error("sendStickerPack: tidak ada sticker valid");
  ...
  const thumbEnc = await encryptForUpload(
    thumbBuf,
    "thumbnail-sticker-pack",
    packEnc.mediaKey,
  );
  const thumbUpload = await writeTempAndUpload(
    thumbEnc,
    "thumbnail-sticker-pack",
  );
  ...
  return {
    ...
    meta: {
      ...
      thumbnailSha256: i.fileSha256
        ? Buffer.from(i.fileSha256).toString("base64")
        : "",
      thumbnailEncSha256: i.fileEncSha256
        ? Buffer.from(i.fileEncSha256).toString("base64")
        : "",
...
function aiRichTokenizer(code, lang = "javascript") {
  ...
  return {
    ...
    unified_codeBlock: tokens.map((t) => ({
      ...
      type: RICH_TYPE_MAP[t.highlightType] ?? "DEFAULT",
    })),
  };
}

function aiRichTable(
  arr,
  ...
  const unified_rows = [
    ...
    const markdown_cells = row.cells.map((cell) => {
      ...
      return {
        text: ex.text,
        ...(ex.inline_entities.length
          ? { inline_entities: ex.inline_entities }
          : {}),
      };
    });
    return {
...
async function resolveMedia(sock, content, mime) {
  ...
  const media = await prepareWAMessageMedia(payload, {
    upload: sock.waUploadToServer,
  });
  const key = `${mime}Message`;
  if (mime === "document" && content.fileName)
    media[key].fileName = content.fileName;
  if (mime === "document" && content.mimetype)
    media[key].mimetype = content.mimetype;
...
function bindButton(sock) {
  sock.sendButton = async (jid, content = {}, options = {}) => {
    ...
    const isStructured =
      isObj(content.header) || isObj(content.body) || isObj(content.footer);
...
function bindWrapper(sock) {
  ...
  sock.sendWithThumbnail = async (
    ...
    if (Array.isArray(restData?.mentions)) {
      for (const j of restData.mentions)
        if (j && typeof j === "string") mentionedJid.add(j.trim());
    }
    ...
    if (thumbnailUrl) {
      try {
        const { image, meta } = await uploadLinkThumb(sock, thumbnailUrl, {
          h: 736,
          w: 1308,
        });
    ...
    if (faviconUrl) {
      try {
        const { meta } = await uploadLinkThumb(sock, faviconUrl, {
          h: 48,
          w: 48,
        });
  ...
  sock.sendAiRich = async (jid, data = {}, options = {}) => {
      ...
      switch (s.type) {
        ...
        case "code": {
          ...
          if (s.code) {
            ...
            meta = {
              codeBlock: blocks,
              unified_codeBlock: blocks.map((b) => ({
                content: b.codeContent,
                type: RICH_TYPE_MAP[b.highlightType] ?? "DEFAULT",
              })),
            };
          }
          builtSubmessages.push({
            messageType: 5,
            codeMetadata: { codeLanguage: lang, codeBlocks: meta.codeBlock },
          });
          sections.push(
            newRichLayout("Single", {
              language: lang,
        ...
        case "table": {
          ...
          if (
            ...
            rows = (s.rows || []).map((r) => {
              ...
              return {
                items: r.items ?? r.cells ?? [],
                isHeading: r.isHeading ?? r.header ?? false,
              };
            });
            unified_rows = rows.map((r) => ({
              is_header: r.isHeading || false,
              cells: r.items,
            }));
          }
          builtSubmessages.push({
            messageType: 4,
            tableMetadata: { title: s.title || "", rows },
          });
          sections.push(
            newRichLayout("Single", {
              rows: unified_rows,
        ...
        case "grid": {
          ...
          if (s.images) {
            list = s.images.map((img) => ({
              ...
              imageHighResUrl:
                img.highResUrl || img.previewUrl || img.url || "",
          ...
          builtSubmessages.push({
            ...
            gridImageMetadata: {
              gridImageUrl: {
                imagePreviewUrl: s.previewUrl || list[0]?.imagePreviewUrl || "",
              },
              imageUrls: list,
            },
          });
        ...
        case "reels": {
          const items = (s.items || []).map((item) => ({
            ...
            profileIconUrl:
              item.profileIconUrl || item.avatarUrl || item.profile || "",
        ...
        case "suggest": {
          const texts = Array.isArray(s.text)
            ? s.text
            : [s.text || s.suggest].filter(Boolean);
          ...
          sections.push(
            newRichLayout(
              layout,
              layout === "Single" ? primitives[0] : primitives,
        ...
        case "product": {
          ...
          const primitives = items.map((item) => ({
            ...
            additional_images: [
              { url: item.icon_url || item.iconUrl || item.icon || "" },
            ],
          ...
          sections.push(
            newRichLayout(
              isArray ? "HScroll" : "Single",
              isArray ? primitives : primitives[0],
            ),
          );
          break;
        }
        ...
        case "post": {
          ...
          const primitives = items.map((p) => ({
            ...
            username: p.username ?? "",
            profile_picture_url:
              p.profile_picture_url || p.profileUrl || p.profile || "",
    ...
    if (allRichSources.length)
      botMetadata.richResponseSourcesMetadata = { sources: allRichSources };
  ...
  sock.sendStickerPack = (jid, data, options) =>
    sendStickerPack(sock, jid, data, options);
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        ff.on("close", (code) => {
          if (code !== 0) {
            return fail(
              new Error(`ffmpeg exit ${code}: ${stderr.slice(-200)}`),
            );
          }
        });
...
class Button extends BaseBuilder {
  ...
  async build(jid, { ...options } = {}) {
    ...
    const userJid =
      this.#client.user?.id || this.#client.authState?.creds?.me?.id || jid;
          ...
          richResponseMessage: {
            ...
            unifiedResponse: {
              data: includesUnifiedResponse
                ? Buffer.from(
                    JSON.stringify({ response_id: randomUUID(), sections }),
                  ).toString("base64")
                : "",
            },
            contextInfo: {
