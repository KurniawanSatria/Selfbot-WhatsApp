...
async function getLatestVersion(repo) {
  try {
    ...
    global.log?.info(
      `Auto-update: nothing released on GitHub (${e.message}) — using latest commit instead`,
    );
  }
  try {
    const data = await fetchJson(
    ...
    global.log?.warn(`Auto-update: failed to check commit (${e.message})`);
  }
  return null;
}
...
async function pullUpdate(branch) {
  try {
    ...
    global.log?.error(`Auto-update: git pull failed (${e.message})`);
    return false;
  }
}
...
async function runUpdate(cfg) {
  ...
  if (!result.hasUpdate) {
    global.log?.info(`Auto-update: already up to date (v${result.local})`);
    return false;
  }
  global.log?.warn(
    `Auto-update: new version available (${result.latest}) — updating...`,
  );
