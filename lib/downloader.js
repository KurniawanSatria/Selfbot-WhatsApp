...
function run(bin, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill("SIGKILL");
      reject(
        new Error(`Timeout: ${bin} tidak selesai dalam waktu yang ditentukan`),
      );
    ...
    child.on("error", (err) => {
      ...
      reject(
        new Error(
          `Gagal menjalankan "${bin}": ${err.message} (pastikan binary tersedia di PATH)`,
        ),
      );
    });
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        reject(
          new Error(
            `${bin} exit code ${code}: ${stderr.slice(-500) || stdout.slice(-500)}`,
          ),
        );
      } else {
...
async function ytmp3(url) {
  ...
  if (!fs.existsSync(expectedPath)) {
    ...
    return {
      ...
      cleanup: () =>
        fs.promises.unlink(path.join(TMP_DIR, files[0])).catch(() => {}),
    };
  }
  return {
...
async function pollStatus(statusUrl, headers) {
  ...
  if (typeof r.data === "string") {
    throw new Error(
      `Status endpoint tidak mengembalikan JSON (status ${r.status}): ${r.data.slice(0, 150)}`,
    );
  }
  return r.data;
}
...
async function instaDL(url, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const retryable =
        [502, 503, 504].includes(status) || err.code === "ECONNABORTED";

      console.error(
        `[instaDL] Attempt ${attempt} gagal:`,
