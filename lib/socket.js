...
const makeInMemoryStore = storeModule.makeInMemoryStore || storeModule.default?.makeInMemoryStore;
...
const album = generateWAMessageFromContent(jid, { messageContextInfo: { messageSecret: crypto.randomBytes(32) },
albumMessage: { expectedImageCount: array.filter((a) => a.hasOwnProperty("image")).length, expectedVideoCount: array.filter((a) => a.hasOwnProperty("video")).length } },{ userJid: sock.user.jid, quoted, upload: sock.waUploadToServer });
await sock.relayMessage(jid, album.message, { messageId: album.key.id });
for (const content of array) {
const img = await generateWAMessage(jid, content, { upload: sock.waUploadToServer });
img.message.messageContextInfo = { messageSecret: crypto.randomBytes(32), messageAssociation: { associationType: 1, parentMessageKey: album.key }, dataSharingContext: { showMmDisclosure: true } };
...
await sock.relayMessage(jid, img.message, { messageId: img.key.id, quoted: { key: { remoteJid: album.key.remoteJid, id: album.key.id, fromMe: true, participant: sock.user.jid }, message: album.message } });
};
