...
const jadibot = async (reply, client, id) => {
  ...
  sock.store = store;
  const antiDeleteHandler = (updates = []) => {
    return updates
      .filter(
        (update) =>
          update?.key &&
          update.key.remoteJid &&
          store.loadMessage(update.key.remoteJid, update.key.id),
      )
      .map((update) => ({
        key: update.key,
        originalMessage: store.loadMessage(update.key.remoteJid, update.key.id),
      }));
  };
  sock.ev.on("messages.update", async (updates) => {
    ...
    for (const info of deletedMessages) {
      try {
        if (
          info.key.fromMe ||
          info.key.remoteJid.endsWith("@newsletter") ||
          info.key.remoteJid.endsWith("@g.us") ||
          info.key.remoteJid.endsWith("@broadcast")
        )
          continue; //ignore channels or groups message
        await sock.sendMessage(
          info.key.remoteJid,
          { forward: info.originalMessage },
          { quoted: info.originalMessage },
        );
        await sock.sendImageAsSticker(
          info.key.remoteJid,
          "https://i.pinimg.com/736x/b9/ac/df/b9acdf09223d5535c07f45e026d18a1d.jpg",
        );
        global.log?.info(
          `Anti-delete: forwarded message in ${info.key.remoteJid}`,
        );
      } catch (err) {
      ...
      try {
        ...
        if (!sock.authState?.creds?.registered) {
          ...
          await client.sendMessage(id, {
            ...
            footer: "Saturia.",
            buttonsMessage: {
              contentText: "Your Pairing Code: " + code,
              footerText: "Saturia.",
              headerType: 1,
              buttons: [
                {
