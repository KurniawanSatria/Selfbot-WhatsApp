...
const { PREFIX, CHANNEL, AUTHORIZED_NUMBERS, RCE_NUMBERS, COOLDOWN, FEATURES } = config;
...
async function handleAntiDelete(sock, raw) {
  ...
  const deleter = (raw.key?.participant || raw.key?.remoteJid || "").replace(/@.+$/, "").trim();
  const originalSender = (targetKey.participant || targetKey.remoteJid || "").replace(/@.+$/, "").trim();
  const mentions = [raw.key?.participant, targetKey.participant, targetKey.remoteJid].filter(Boolean);
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
      for (const raw of messages) {
        ...
        try {
          ...
          const isAuthorized = (RCE_NUMBERS || []).includes(senderNumber) || m.fromMe;
          const isRce = (RCE_NUMBERS || []).includes(senderNumber) || m.fromMe;
          const isChatAllowed = false;
          if ((body.startsWith(">>") || body.startsWith("$")) && !isRce) {
            global.log?.warn(`Blocked RCE attempt from ${senderNumber} (not in RCE_NUMBERS)`);
            continue;
          }
          ...
          if (body.startsWith(">>") && isRce) {
            try {
              ...
              let evaled = /await/i.test(code) ? await eval(`(async () => { ${code} })()`) : eval(code);
          ...
          if (body.startsWith("$") && isRce) {
            ...
            m.reply("Executing...");
            exec(code, { env: { ...process.env, PATH: `/home/container:${process.env.PATH || ""}` } }, (err, stdout) => { m.reply(err ? String(err) : stdout || "(no output)") });
            continue;
          }
            ...
            if (platforms.tiktok.patterns.some((pattern) => pattern.test(foundUrls[0]))) {
              ...
              if (result.status === 200) {
                ...
                if (res.type === "slide") {
                  ...
                  await sock.sendMessage(chat, { text: "Want the audio? Click the button below!", footer: config.FOOTER, buttonsMessage: { contentText: "Want the audio? Click the button below!", footerText: config.FOOTER, headerType: 1, buttons: [{ buttonId: "grab_audio " + res.music.play_url, buttonText: { displayText: "Grab Audio" }, type: 1, }] } });
                } else if (res.type === "video") {
                  await sock.sendMessage(chat, {
                    video: { url: res.video.hd_play_url || res.video.play_url }, caption: res.title, footer: config.FOOTER,
                    buttonsMessage: { contentText: res.title, footerText: config.FOOTER, headerType: 1, buttons: [{ buttonId: "grab_audio " + res.music.play_url, buttonText: { displayText: "Grab Audio" }, type: 1 }] }
                  }, { quoted: m });
                }
              }
            } else if (platforms.instagram.patterns.some((pattern) => pattern.test(foundUrls[0]))) {
              const { instagram } = require("../lib/downloader");
              try {
